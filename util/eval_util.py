...
def compute_metric(metric, model, dataset, dataloader, batch_tnf, batch_size, two_stage=True, do_aff=False,
                   do_tps=False, args=None):
    # Initialize stats
    N = len(dataset)
    stats = {}
    ...
    for i, batch in enumerate(dataloader):
        ...
        if metric == 'inlier_count':
            stats = inlier_count(batch, batch_start_idx, theta_aff, theta_tps, theta_aff_tps, corr_aff, corr_aff_tps,
                                 stats, args)
...
def point_dist_metric(batch, batch_start_idx, theta_aff, theta_tps, theta_aff_tps, stats, args, use_cuda=False):
...
def inlier_count(batch, batch_start_idx, theta_aff, theta_tps, theta_aff_tps, corr_aff, corr_aff_tps, stats, args,
                 use_cuda=False):
    inliersComposed = TwoStageWeakInlierCount(use_cuda=torch.cuda.is_available(), dilation_filter=0,
...
def pck_metric(batch, batch_start_idx, theta_aff, theta_tps, theta_aff_tps, stats, args, use_cuda=False):
...
def pascal_parts_metrics(batch, batch_start_idx, theta_aff, theta_tps, theta_aff_tps, stats, args, use_cuda=False):
    do_aff = theta_aff is not None
    do_tps = theta_tps is not None
    do_aff_tps = theta_aff_tps is not None
    ...
    for b in range(batch_size):
        ...
        source_mask = Variable(
            torch.FloatTensor(batch['part_A'][b].astype(np.float32)).unsqueeze(0).transpose(2, 3).transpose(1, 2))
        target_mask = Variable(
            torch.FloatTensor(batch['part_B'][b].astype(np.float32)).unsqueeze(0).transpose(2, 3).transpose(1, 2))
...
def area_metrics(batch, batch_start_idx, theta_aff, theta_tps, theta_aff_tps, stats, args, use_cuda=False):
    do_aff = theta_aff is not None
    do_tps = theta_tps is not None
    do_aff_tps = theta_aff_tps is not None
    ...
    for b in range(batch_size):
        ...
        if do_aff_tps:
            grid_aff_tps = pointsToGrid(pt.affPointTnf(theta_aff[b, :].unsqueeze(0),
                                                       pt.tpsPointTnf(theta_aff_tps[b, :].unsqueeze(0), grid_XY_vec)))
            warped_mask_aff_tps = F.grid_sample(source_mask, grid_aff_tps)
            ...
            stats['aff_tps']['localization_error'][idx] = localization_error(source_mask_np, target_mask_np,
                                                                             flow_aff_tps)

    return stats


def flow_metrics(batch, batch_start_idx, theta_aff, theta_tps, theta_aff_tps, stats, args, use_cuda=False):
...
def poly_str_to_mask(poly_x_str, poly_y_str, out_h, out_w, use_cuda=False):
...
def intersection_over_union(warped_mask, target_mask):
    relative_part_weight = torch.sum(torch.sum(target_mask.data.gt(0.5).float(), 2, True), 3, True) / torch.sum(
        target_mask.data.gt(0.5).float())
    part_iou = torch.sum(torch.sum((warped_mask.data.gt(0.5) & target_mask.data.gt(0.5)).float(), 2, True), 3,
                         True) / torch.sum(
        torch.sum((warped_mask.data.gt(0.5) | target_mask.data.gt(0.5)).float(), 2, True), 3, True)
